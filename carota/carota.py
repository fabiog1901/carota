...
class FabDate:
    def __init__(self, rows, start, delta, format, seed):
        self.rows = rows
...
class FabInt:
    def __init__(self, rows, start, end, seed):
        self.rows = rows
        self.rd = random.Random()
...
class FabString:
    def __init__(self, rows, size, seed):
        self.rows = rows
        self.rd = random.Random()
        self.size = int(size)
        if seed is not None:
...
class FabChoices:
    def __init__(self, rows, choices, weights, seed):
        self.rows = rows
        self.rd = random.Random()
...
class FabFirstName:
    def __init__(self, rows, gndr, seed):
        self.rows = rows
        self.rd = random.Random()
        global gender
        self.gender = gender
...
def get_fields(text, rows):
    ...
    for f in fields:
        ...
        if f[0] == 'int':
            f[1] = FabInt(rows, f[1].get('start', INT_START), f[1].get('end', INT_END), f[1].get('seed', None)).generator
        
        elif f[0] == 'index':
            f[1] = FabIndex(rows, f[1].get('start', 1)).generator
        
        elif f[0] == 'constant':
            f[1] = FabConstant(rows, f[1].get('value', None)).generator

        elif f[0] == 'uuid':
            f[1] = FabUUID(rows, f[1].get('seed', None)).generator
        
        elif f[0] == 'ssn':
            f[1] = FabSSN(rows, f[1].get('seed', None)).generator
        
        elif f[0] == 'tel':
            f[1] = FabTel(rows, f[1].get('seed', None)).generator

        elif f[0] == 'lastname':
            f[1] = FabLastName(rows, f[1].get('seed', None)).generator

        elif f[0] == 'date':
            f[1] = FabDate(rows, f[1].get('start', None), f[1].get('delta', DATE_DELTA), f[1].get('format', DATE_FORMAT), f[1].get('seed', None)).generator
        
        elif f[0] == 'choices':
            f[1] = FabChoices(rows, f[1]['list'].split(" "), f[1].get('weights', None), f[1].get('seed', None)).generator

        elif f[0] == 'string':
            f[1] = FabString(rows, f[1].get('size', STRING_SIZE), f[1].get('seed', None)).generator

        elif f[0] == 'gender':
            f[1] = FabGender(rows, f[1].get('seed', None)).generator

        elif f[0] == 'firstname':
            f[1] = FabFirstName(rows, f[1].get('gender', None), f[1].get('seed', None)).generator
...
def carota(rows, 
    ...
    fields = get_fields(text, rows)

    for res in multiproc(fields):
...
def multiproc(fields):
    ...
    for i, f in enumerate(fields):
        processes.append(
            mp.Process(target=_worker, args=(i, barrier, queue, f[1], ()))
        )
